...
func Handle330(b Router, msg ircmsg.Message) error {
	// Forward message to client
	b.BroadcastToClients(msg)
	return nil
}
...
func Handle301(b Router, msg ircmsg.Message) error {
	...
	if len(msg.Params) > 2 {
		...
		log.Trace().Msgf("[upstream %s] Away message for %s receieved from WHOIS", b.GetUpstreamConn().Server, nick)
